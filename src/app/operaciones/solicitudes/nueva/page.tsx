...
export default function NuevaSolicitudOperaciones() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (insertError) {
      ...
      setTimeout(() => router.push('/operaciones/solicitudes'), 2000)
    }
  }
    ...
    <main className="p-6 max-w-xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <label className="block">Cliente:
          <select value={clienteId} onChange={(e) => setClienteId(e.target.value)} required className="w-full mt-1 p-2 border rounded">
