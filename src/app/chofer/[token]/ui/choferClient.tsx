...
function Item({ e, onMark }: any) {
  return (
    <div className="border rounded p-3 mb-2">
      <div className="font-medium">#{e.orden} — {e.cliente_entrega ?? e.subcliente}</div>
      <div>{e.direccion}, {e.localidad}, {e.provincia}</div>
      <div>Remitos: {e.remitos ?? e.remito ?? '-'}</div>
...
export default function ChoferClient({ token, initialEntregas }: any) {
  ...
  const done = items.filter((i: any) => (i.estado_entrega ?? i.estado) === 'entregado').length
  ...
  async function onMark(id: string, estado: 'entregado'|'rechazado') {
    ...
    if (r.ok) {
      setItems((prev: any[]) => prev.map(i =>
        i.id === id
          ? { ...i, estado_entrega: estado, estado, fecha_entrega_real: new Date().toISOString(), completado_a: new Date().toISOString() }
          : i
      ))
    }
  }
